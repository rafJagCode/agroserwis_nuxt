<template>
  <div class="department-form">
    <v-form v-model="valid" ref="form" lazy-validation>
      ...
      <SlideSelector
        v-if="department.slides"
        :slides="availableSlides"
        :selected="department.slides"
...
export default {
  ...
  data: () => ({
    ...
    valid: true,
    nameRules: [(v) => !!v || 'Nazwa oddziału jest wymagana'],
    streetRules: [(v) => !!v || 'Ulica jest wymagana'],
    addressRules: [(v) => !!v || 'Adres jest wymagany'],
    emailRules: [
      (v) =>
        !v ||
  ...
  methods: {
    async update() {
      ...
      await this.$axios.post(
        `/api/update-department/${this.department.link}`,
        this.department
      )
